    ...
    if(req.query.notice_num === undefined){
        ...
        .then(result => {
            if(result[0] !== undefined){
                ...
                res.render("notice/notice_board", { obj : { id : result[0].n_id, title : result[0].n_title, content : result[0].n_content, createdAt : result[0].createdAt } } );
            }else{
                res.send("<h1>니가 찾고 싶은거는 없어 돌아가</h1>");
            }
        })
        .catch(err => {
...
router.get('/edit', (req, res) => {
    console.log("/notice", req.route.path);
    res.render('notice/notice_edit', { obj : { title : "공지사항 작성" } } );
});

router.post('/create', (req, res) => {
    Promise.all([noticeController.create(req, res)])
    .then(result => {
        console.log("/notice", req.route.path);
        ...
        if(result[0]){
            ...
            res.send( { content : "생성 완료", link : "/notice/edit" } );
        }
